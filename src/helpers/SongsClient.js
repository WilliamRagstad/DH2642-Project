...
class SongsClient {
    ...
    search(q) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let result = [];
                if (this.key) {
                    const res = await axios.get(`https://cors-anywhere.herokuapp.com/${CONSTANTS.BASE_URL}/search?q=${term}`, {
                    ...
                    const res = await axios.get(`https://cors-anywhere.herokuapp.com/${CONSTANTS.UN_BASE_URL}/search/multi?per_page=5&q=${term}`, this.config.requestOptions);
    ...
    get(q) {
        ...
        return new Promise((resolve, reject) => {
            axios.get(`https://cors-anywhere.herokuapp.com/${CONSTANTS.BASE_URL}/songs/${q}`, {
                ...this.config.requestOptions,
                headers: {
                    'Authorization': `Bearer ${this.key}`
                }
